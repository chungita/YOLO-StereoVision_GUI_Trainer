...
class StereoTrainingModule(BaseModule):
    ...
    def create_tab(self):
        ...
        RAFT-Stereo 立體視覺訓練說明 (基於 Steps 訓練)：
        ...
        2. 支持的預訓練模型：
           - raftstereo-sceneflow.pth：Scene Flow數據集訓練
           - raftstereo-middlebury.pth：Middlebury數據集訓練
           - raftstereo-eth3d.pth：ETH3D數據集訓練
           - iraftstereo_rvc.pth：RVC數據集訓練
           - raftstereo-realtime.pth：實時版本
        
        3. 訓練參數建議：
           - 批次大小：6（train_stereo.py默認）
           - 訓練步數：100000步（可調整，建議範圍：50000-500000）
           - 訓練迭代：16次（每次前向傳播的迭代次數）
           - 驗證迭代：32次（驗證時的迭代次數）
        
        4. 圖像尺寸選擇：
           - 640x480 (原始尺寸)：使用數據集原始尺寸，適合GPU內存充足的場景
           - 320x720：較小的裁剪尺寸，訓練速度更快，內存占用更少
           - 注意：較大的圖像尺寸需要更多GPU內存，可能需要減小批次大小
        ...
        6. 輸出：
           - 訓練好的模型保存在checkpoints目錄
           - 訓練日誌記錄在運行日誌中
           - TensorBoard日誌保存在runs目錄
        """)
    ...
    def start_stereo_training(self):
        ...
        self.log(f"   圖像尺寸: {image_size[0]}x{image_size[1]} (width x height)")
