...
class Logger:
    ...
    def __init__(self, model, scheduler, output_dir='.'):
        ...
        self.writer = SummaryWriter(log_dir=str(Path(output_dir) / 'runs'))
    ...
    def _print_training_status(self):
        ...
        if self.writer is None:
            self.writer = SummaryWriter(log_dir=str(Path(self.output_dir) / 'runs'))
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(tqdm(train_loader)):
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                save_path = Path(args.output_dir) / 'checkpoints' / f'{total_steps + 1}_{args.name}.pth'
        ...
        if len(train_loader) >= 10000:
            save_path = Path(args.output_dir) / 'checkpoints' / f'{total_steps + 1}_epoch_{args.name}.pth.gz'
            logging.info(f"Saving file {save_path}")
            torch.save(model.state_dict(), save_path)

    print("FINISHED TRAINING")
    logger.close()
    PATH = Path(args.output_dir) / 'checkpoints' / f'{args.name}.pth'
    ...
    def train(self, progress_callback=None):
        ...
        class Args:
            def __init__(self, config):
                ...
                # 處理 image_size（轉換為 crop_size 用於增廣）
                # 如果 image_size 是列表或元組，使用它作為 crop_size
                if hasattr(config, 'image_size') and config.image_size:
                    if isinstance(config.image_size, (list, tuple)) and len(config.image_size) == 2:
                        ...
                        # crop_size 用於增廣（需要反轉高度和寬度，因為是 (H, W)）
                        self.crop_size = (config.image_size[1], config.image_size[0])  # (height, width)
                    else:
                        self.image_size = config.image_size
                        self.crop_size = None
                else:
