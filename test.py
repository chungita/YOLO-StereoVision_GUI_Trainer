#!/usr/bin/env python3
"""
Test script to run RAFT-Stereo demo with Middlebury testF dataset
執行 RAFT-Stereo 演示，使用 Middlebury testF 數據集
"""
...
class RAFTStereo(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        if USE_NEW_AUTOCAST:
            autocast_context = autocast('cuda', enabled=self.args.mixed_precision)
        else:
            autocast_context = autocast(enabled=self.args.mixed_precision)
...
def demo(args):
    ...
    try:
        ...
        print("成功加載預訓練模型 / Successfully loaded pretrained model")
    ...
    with torch.no_grad():
        ...
        print(f"找到 {len(left_images)} 張圖像 / Found {len(left_images)} images")
        print(f"保存文件到 {output_directory}/")
...
def run_demo():
    ...
    # 檢查數據集路徑是否存在
    left_imgs_path = "datasets/Img0/test/Forest_Video_1_Img0_10.png"
    right_imgs_path = "datasets/Img1/test/Forest_Video_1_Img1_10.png"
    
    # 創建參數對象
    args = Namespace(
        restore_ckpt=model_path,
        save_numpy=False,
        left_imgs=left_imgs_path,
        right_imgs=right_imgs_path,
    ...
    print("準備開始處理...")
    print("Prepare to start processing...")
    print(f"模型路徑 / Model path: {model_path}")
    print(f"左圖路徑 / Left images: {left_imgs_path}")
    print(f"右圖路徑 / Right images: {right_imgs_path}")
    print("-" * 50)
...
if __name__ == "__main__":
    print("RAFT-Stereo 測試腳本")
    print("RAFT-Stereo Test Script")
    print("=" * 50)
